...
photo = PhotoImage(file="images\musicn1.png")
...
name_entry = tk.Entry(root,
                      textvariable=name1, font=('calibre', 10, 'normal'),bg='black',fg='white')
...
def get_results(emotion_code):
    ...
    calm_set=[]
    if emotion_code == 0:
        sad_set.append(df1[df1['kmeans'] == 0]['name'].head(NUM_RECOMMEND))
        return pd.DataFrame(sad_set).T
    elif emotion_code == 1:
        happy_set.append(df1[df1['kmeans'] == 1]['name'].head(NUM_RECOMMEND))
        return pd.DataFrame(happy_set).T
    elif emotion_code == 2:
        anger_set.append(df1[df1['kmeans'] == 2]['name'].head(NUM_RECOMMEND))
        return pd.DataFrame(angry_set).T
    else:
        calm_set.append(df1[df1['kmeans'] == 3]['name'].head(NUM_RECOMMEND))
...
def final():
    ...
    elif emotion_word=='anger':
        print('emotion detected is ANGER')
    else:
        print('emotion detected is NEUTRAL/CALM')
...
image1 = Image.open("images\musicn2.jpg")
test = ImageTk.PhotoImage(image1)

label1 = tk.Label(image=test)
label1.image = test
label1.place(x=650, y=250) #470,150
root.mainloop()
window.mainloop()
